...
from models import build_improved_cnn
...
def main():
    ...
    model_path = os.path.join(SAVED_MODEL_DIR, "improved_cnn_model.keras")
    ...
    with open(os.path.join(result_dir, "result.txt"), "w", encoding="utf-8") as f:
        f.write(f"Improved CNN Test Loss: {test_loss:.4f}\n")
        f.write(f"Improved CNN Test Accuracy: {test_acc:.4f}\n")
